...
class IntegrationTestsApp(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.test_files = [os.path.join(root, file)
                          for root, _, files in os.walk(cls.path)
                          for file in files]
        cls.test_drs = [os.path.join(root, dr)
    ...
    def tearDownClass(cls):
        ...
        drs = [os.path.join(root, dr)
               for root, drs, _ in os.walk(cls.path)
               for dr in drs]
        # `os.rmdir` removes only empty directories,
        # so we need start deleting from inner subdirectories
    ...
    def test_synchronize_files(self):
        old_photos_paths = [photo.get_file_path(self.path)
                            for photo in self.photos]
        with self.app.dao:
            self.app.dao.save_photos(self.photos)
        self.app.synchronize_files(self.path)
        self.assertTrue(all(os.path.exists(old_photo_path)
                            for old_photo_path in old_photos_paths))

        for photo in self.photos:
            photo.album = photo.album[::-1] or 'test'
        new_photos_paths = [photo.get_file_path(self.path)
                            for photo in self.photos]
        self.assertTrue(all(not os.path.exists(new_photo_path)
                            for new_photo_path in new_photos_paths))
        self.app.dao.save_photos(self.photos)
        self.app.synchronize_files(self.path)
        self.assertTrue(all(not os.path.exists(old_photo_path)
                            for old_photo_path in old_photos_paths))
        self.assertTrue(all(os.path.exists(new_photo_path)
                            for new_photo_path in new_photos_paths))
