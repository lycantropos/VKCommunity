...
class UnitTestsDataAccess(unittest.TestCase):
    def setUp(self):
        self.raw_filters = dict(owner_id='-14',
...
class UnitTestsExceptionsDataAccess(unittest.TestCase):
    ...
    def test_check_filters_bad_datetime_parameters(self):
        self.assertRaises(
            ValueError, check_filters, dict(
                start_datetime=calendar.timegm(datetime.datetime.max.utctimetuple()) + 1)
        )
        self.assertRaises(
            ValueError, check_filters, dict(
                start_datetime=calendar.timegm(datetime.datetime.min.utctimetuple()) - 1)
        )
        self.assertRaises(TypeError, check_filters,
                          dict(start_datetime='2154-10-30 21:49:55'))
